...
$$ x = \alpha \ x_{new} + (1-\alpha) \ stop\_grad(x_{old}),$$
where:

* $x_{old}$ - old value of the weight before recycling
* $x_{new}$ - new value chosen for the weight
* $\alpha$ - non-trainable parameter, changed linearly from 0 to 1 in the steps following reinitialization <!--dodać info o n kroków-->
...
$$ Loss = \sum_{l = 1}^{L} \sum_{n = 1}^{d_\text{ff}} dist(M_{l,n}, stop\_grad(avg_n(M_{l,n})))$$
where:
