...
{{< include plots_jk/smooth_fig.qmd >}}
...
$$ Loss = \sum_{l = 1}^{L} \sum_{n = 1}^{dff} dist(M_{l,n}, stop\_grad(midpoint(M_{l,*}))), $$
where:

* $M_{l,n}$ -magnitude of then-th neuron in the l-th layer
*$dist$ - some distance function, typically$l_1$,$l_2$
*$midpoint$ -mean or median. Median can be used instead of mean because of itsrobustness with respect
