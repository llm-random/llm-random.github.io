...
Aiming to avoid these problems we propose **Mixture of Tokens**: a new, fully-differentiable type of architecture that retains all efficiency benefits of MoE and alleviates the aformentioned problems by _mixing tokens_ before feeding them into the FeedForward layer instead of routing tokens to experts, in effect allowing the model to simultaneously learn from all token-expert combinations. Crucially, this technique is fully compatible with both masked and causal LLM training. 
...
* **Load imbalance.** Typically, in MoE we set the maximum *capacity* for each expert. However, we are not able to efficiently restrict the choice of the routing network to assign tokens in a perfectly balanced way. This leads to *token dropping* (when some tokens are not processed at all) and *expert under-training* [mention underspecialization as well]{.SJ} (when some experts receive less data and are therefore not trained enough). [there is work by Basil that does some balancing for that. Will liink later]{.SA}
* **Information leak.** Some of the most successful methods in the area[sth else here ]{.SA} process tokens from different positions in a sequence together (i.e. by comparing scores of all tokens in a batch). This imposes an information leak, and ~~makes it impossible to use such techniques in autoregressive decoding~~[hinders their utility in in autoregressive decoding]{.SA} [remember that we leak in another way, so don't be too harsh here]{.SA}, covering a large part of use cases in modern language models. [SJ edit]{.SJ}<!--[SIMON ANOTHER LINK!!!!!!]-->
...
# Method
In this section we provide an overview of our proposed method. More detailed description of all operations can be found in the next section. [T U T A J L I N K]
...
In Mixture of Tokens, all experts work independently from one another and act similar to a normal feedforward layer. The difference in our setup is that a single expert processes weighted average of group of tokens. Then, output from an expert is rescaled and redistributed into group of tokens. See the diagrams below. [SA stylistycznie]{.SA}
...
# Experiments
## Experimental setup
We train a standard GPT-like model on the language modeling task using cross entropy loss on the C4 dataset [@2019t5].
...
Our model trains on group size of 32, with 32x more parameters in the FeedForward layer compared to the baseline, distributed among 512 experts per layer. [SA TODO rephrase]{.SA}
...
# Next steps
In the near future, we're planning to step up our efforts with several improvements we'll be examining in this section.

## Scaling Up 
In a previous section <TODO check> we showcased results for a BERT-mini FLOP matched Mixture of Tokens. Our preliminary experiments are also suggesting even better outcomes for larger models. In upcoming weeks we aim to prepare comprehensive comparison of larger models with FLOP matched Mixture of Tokens models. [TODO SJ do przepisania] {.SJ}

## From Mixture of Tokens to Expert-choice
Napsiać że emprucznie wyszło, że w zasadzie to się nam to samo dyskretytuje i dzieki temu łatwo będzie pozbyć się leaka informacyjnego. Napisać o caveat'ach, ze pierwszy blog się nie dyskrezytuje ale pracujemy nad tym i dodamy potem
...
As our results in this post covers only GPT model, in future work we plan to compare results of Mixture of Tokens on BERT model.  -->
